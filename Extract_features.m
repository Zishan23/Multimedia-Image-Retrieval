...
[meanAmplitude, msEnergy] = gaborWavelet(img, 4, 6); % 4 scales, 6 orientations
wavelet_moments = waveletTransform(queryImage);
% construct the queryImage feature vector
queryImageFeature = [hsvHist autoCorrelogram color_moments meanAmplitude,msEnergy wavelet_moments str2double(name)];
pause(0.25)
waitbar(1,WB,'Done')
close(WB)
end
